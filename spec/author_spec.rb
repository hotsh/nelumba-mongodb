...
describe Lotus::Author do
  describe "Schema" do
    it "should have a local key" do
      Lotus::Author.keys.keys.must_include "local"
    end

    it "should have a uid" do
      Lotus::Author.keys.keys.must_include "uid"
    end

    it "should have a nickname" do
      Lotus::Author.keys.keys.must_include "nickname"
    end

    it "should have an extended_name" do
      Lotus::Author.keys.keys.must_include "extended_name"
    end

    it "should have a uri" do
      Lotus::Author.keys.keys.must_include "uri"
    end

    it "should have an email" do
      Lotus::Author.keys.keys.must_include "email"
    end

    it "should have a name" do
      Lotus::Author.keys.keys.must_include "name"
    end

    it "should have an organization" do
      Lotus::Author.keys.keys.must_include "organization"
    end

    it "should have an address" do
      Lotus::Author.keys.keys.must_include "address"
    end

    it "should have a gender" do
      Lotus::Author.keys.keys.must_include "gender"
    end

    it "should have a note" do
      Lotus::Author.keys.keys.must_include "note"
    end

    it "should have a display_name" do
      Lotus::Author.keys.keys.must_include "display_name"
    end

    it "should have a preferred_username" do
      Lotus::Author.keys.keys.must_include "preferred_username"
    end

    it "should have a birthday" do
      Lotus::Author.keys.keys.must_include "birthday"
    end

    it "should have an anniversary" do
      Lotus::Author.keys.keys.must_include "anniversary"
    end

    it "should have a created_at" do
      Lotus::Author.keys.keys.must_include "created_at"
    end

    it "should have a updated_at" do
      Lotus::Author.keys.keys.must_include "updated_at"
    end
  end
  ...
  describe "find_or_create_by_uid!" do
    it "should allow Lotus::Author to be given" do
      ...
      author.stubs(:uid).returns("UID")

      Lotus::Author.expects(:first).with(has_entry(:uid, "UID")).returns(author)

      Lotus::Author.find_or_create_by_uid! author
    end

    it "should allow a uid to be passed" do
      Lotus::Author.expects(:first).with(has_entry(:uid, "UID")).returns(Lotus::Author.create)

      Lotus::Author.find_or_create_by_uid! :uid => "UID"
    end

    it "should return the existing Lotus::Author object for the Lotus::Author" do
      author = Lotus::Author.new(:uid => "UID")

      matching = Lotus::Author.create(:uid => author.uid)

      Lotus::Author.find_or_create_by_uid!(author).uid.must_equal matching.uid
    end

    it "should return the existing Lotus::Author object for the given uid" do
      matching = Lotus::Author.create(:uid => "UID")

      Lotus::Author.find_or_create_by_uid!(:uid => "UID").uid.must_equal matching.uid
    end

    it "should create a new Lotus::Author when the Lotus::Author is not found" do
      Lotus::Author.expects(:create!)
      Lotus::Author.find_or_create_by_uid! :uid => "unknown"
    end

    it "should return a new Lotus::Author when the Lotus::Author is not found" do
      Lotus::Author.stubs(:create!).returns("author")
      Lotus::Author.find_or_create_by_uid!(:uid => "unknown").must_equal "author"
    end

    it "should account for race condition where entry was created after find" do
      Lotus::Author.stubs(:first).returns(nil).then.returns("author")
      Lotus::Author.stubs(:create!).raises("")
      Lotus::Author.find_or_create_by_uid!(:uid => "UID").must_equal "author"
    end
  end
  ...
  describe "create!" do
    it "should take a Lotus::Author" do
      ...
      Lotus::Author.create!(author).uid.must_equal "UID"
    end

    it "should allow a hash of values" do
      Lotus::Author.create!(:uid => "UID").uid.must_equal "UID"
    end

    it "should not store arbitrary fields" do
      Lotus::Author.create!(:foobar => "bar").serializable_hash.keys
        .wont_include "foobar"
    end
  end
  ...
  describe "discover!" do
    ...
    it "should return Lotus::Author if does not exist" do
      author = stub('Lotus::Author')
      Lotus::Identity.stubs(:find_by_identifier).returns(nil)
      ...
      Lotus::Author.discover!("wilkie@rstat.us").must_equal author
    end

    it "should return existing Lotus::Author if it can" do
      author = stub('Lotus::Author')
  ...
  describe "#discover_feed!" do
    it "should use Lotus to discover a feed from the identity" do
      ...
      identity = Lotus::Identity.create!(:author_id => author.id)
  ...
  describe "#short_name" do
    it "should use display_name over all else" do
      author = Lotus::Author.create(:display_name => "display",
                             :name => "name",
    ...
    it "should use name over all else when display name doesn't exist" do
      author = Lotus::Author.create(:name => "name",
                             :preferred_username => "preferred",
                             :nickname => "nickname",
                             :uid => "unique")
    ...
    it "should use preferred_username when name and display_name don't exist" do
      author = Lotus::Author.create(:preferred_username => "preferred",
                             :nickname => "nickname",
                             :uid => "unique")
    ...
    it "should use nickname when it exists and others do not" do
      author = Lotus::Author.create(:nickname => "nickname",
                             :uid => "unique")
    ...
    it "should use uid when all else fails" do
      author = Lotus::Author.create(:uid => "unique")
  ...
  describe "#remote?" do
    it "should return the negation of the local field" do
      Lotus::Author.create(:local => true).remote?.must_equal false
      Lotus::Author.create(:local => false).remote?.must_equal true
    end
  end

  describe "#local?" do
    it "should return the local field" do
      Lotus::Author.create(:local => true).local?.must_equal true
      Lotus::Author.create(:local => false).local?.must_equal false
    end
  end
